...
records (their device and limits were never recorded), and 5 → 6 the
hash-chained event log. None touches existing rows; reaching 6 logs every
existing record, delivery and replay as an *.imported event, so the chain
covers the ledger from the migration on.
...
def migrate_to_current(conn: sqlite3.Connection, version: int, archive: BlobStore) -> None:
    ...
    # version → DDL that reaches version + 1. Reaching 6 creates the event
    # log, which then records the state found as *.imported events.
    additive = {2: DELIVERY_DDL, 3: REPLAY_DDL, 4: RUN_CONDITIONS_DDL, 5: EVENTS_DDL}
