...
class TestReplay:
    ...
    def test_run_settings_are_recorded(self, doc: Path, tmp_path: Path) -> None:
        ledger, record = self._record(tmp_path, doc, self._files(self.LAYOUT))
        # FakeBackend applies 512m and 2.5 CPUs; the timeout is the default.
        assert record.run_settings == {
            "device": "cpu", "memory": "512m", "cpus": 2.5, "timeout_seconds": 1800,
        }

    def test_replay_uses_the_recorded_timeout_and_cpus(self, doc: Path, tmp_path: Path) -> None:
        ledger = open_ledger(tmp_path / "ledger.db")
        files = self._files(self.LAYOUT)
        run = run_parser(
            self.POLICY_PARSER, doc, tmp_path / "out", store=ledger.archive,
            timeout_seconds=3600, backend=FakeBackend(write=files),
        )
        record = record_parser_run(ledger, run, source=Source.from_path(doc))
        cpus4 = FakeBackend(write=files)
        cpus4.cpus = 4.0
        run4 = run_parser(
            self.POLICY_PARSER, doc, tmp_path / "out4", store=ledger.archive, backend=cpus4,
        )
        record4 = record_parser_run(ledger, run4, source=Source.from_path(doc))

        result, backend = self._replay(ledger, record, files)
        assert result.outcome is ReplayOutcome.EQUIVALENT
        assert backend.seen.timeout_seconds == 3600
        result, backend = self._replay(ledger, record4, files)
        assert result.outcome is ReplayOutcome.EQUIVALENT
        assert backend.seen.env["OMP_NUM_THREADS"] == "4"

    def test_gpu_record_replays_on_the_gpu_image(self, doc: Path, tmp_path: Path) -> None:
        parser = replace(
            self.POLICY_PARSER, gpu="optional", gpu_image="localhost/stele/fake-gpu:1.0"
        )
        ledger = open_ledger(tmp_path / "ledger.db")
        files = self._files(self.LAYOUT)
        run = run_parser(
            parser, doc, tmp_path / "out", store=ledger.archive,
            backend=FakeBackend(gpu=True, name="oci-runc-gpu", write=files),
        )
        record = record_parser_run(ledger, run, source=Source.from_path(doc))
        assert record.run_settings["device"] == "gpu"

        spec = replay_spec(parser).with_run_settings(record.run_settings)
        oci = spec.backend(record.parser)
        assert oci.image == parser.gpu_image and oci.gpu
        assert spec.requirements.requires_gpu

        gpu = FakeBackend(gpu=True, name="oci-runc-gpu", write=files)
        spec = replay_spec(parser, backend=gpu)
        result = replay_record(ledger, ParserCatalog([spec]), record)
        assert result.outcome is ReplayOutcome.EQUIVALENT
        assert gpu.seen.env["STELE_PARSER_DEVICE"] == "gpu"
    ...
    def test_gpu_record_without_a_gpu_is_unreplayable(self, doc: Path, tmp_path: Path) -> None:
        ...
        cpu_only = FakeBackend(write=files)
        result = replay_record(ledger, ParserCatalog([replay_spec(parser, backend=cpu_only)]), record)
        assert result.outcome is ReplayOutcome.UNREPLAYABLE
        assert cpu_only.seen is None  # never run on the CPU instead

    def test_settings_the_parser_cannot_honour_are_unreplayable(
        self, doc: Path, tmp_path: Path
    ) -> None:
        ledger = open_ledger(tmp_path / "ledger.db")
        files = self._files(self.LAYOUT)
        ...
        record = record_run(
            ...
            parser_config=dict(run.identity.config), source=Source.from_path(doc),
            run_settings={"device": "gpu"},  # PARSER has no GPU build
        )
        result, backend = self._replay(ledger, record, files)
        assert result.outcome is ReplayOutcome.UNREPLAYABLE
        assert "recorded settings" in result.reason
        assert backend.seen is None
