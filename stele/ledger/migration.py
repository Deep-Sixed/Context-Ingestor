...
outcome, and 6 → 7 adds the hash-chained event log. None of these changes
existing rows; reaching 7 logs every existing record, delivery and replay as
an *.imported event, so the chain covers the ledger from the migration on.
...
def migrate_to_current(conn: sqlite3.Connection, version: int, archive: BlobStore) -> None:
    ...
    additive = {2: DELIVERY_DDL, 3: REPLAY_DDL, 4: RUN_CONDITIONS_DDL, 5: V6_DDL, 6: EVENTS_DDL}
