...
| Writes only under `/stele/output` | **Enforced where the kernel has Landlock** — writes to the tmpfs mounts fail with `EACCES`; `writable_scratch=True` additionally allows the ephemeral `/tmp`. Device sinks (`/dev/null`, `/dev/zero`, `/dev/full`) stay writable. Without Landlock the mount layout alone applies: writes outside `/stele/output` never persist |
